...
#'
#' @export
compareNetwork <- function(Inputs,
                         Source = "source",
                         Target = "target",
                         Value = "size",
                         NodeID = "name",
                         ...
                         clickAction = NULL)
{
    ...
    htmlwidgets::createWidget(
            name = "compareNetwork",
            x = list(inputs = Inputs, options = options),
            width = width,
            height = height,
...
#' @export
compareNetworkOutput <- function(outputId, width = "100%", height = "960px") {
        shinyWidgetOutput(outputId, "compareNetwork", width, height,
                          package = "networkD3")
}

#' @rdname networkD3-shiny
#' @export
renderCompareNetwork <- function(expr, env = parent.frame(), quoted = FALSE) {
        if (!quoted) { expr <- substitute(expr) } # force quoted
        shinyRenderWidget(expr, compareNetworkOutput, env, quoted = TRUE)
}
